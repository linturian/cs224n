...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        padded_sents_char = pad_sents_char(sents_char_indices, self.word2id['<pad>'])
        padded_sents_char_tensor = torch.tensor(padded_sents_char, dtype=torch.long, device=device)
        padded_sents_char_tensor = padded_sents_char_tensor.permute(1, 0, 2)
        return padded_sents_char_tensor
        ### END YOUR CODE
