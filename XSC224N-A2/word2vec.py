...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = -outsideVectors[outsideWordIdx] + (p.dot(outsideVectors))

    y = np.zeros(len(outsideVectors))
    y[outsideWordIdx] = 1
    gradOutsideVecs = (p - y).reshape(len(outsideVectors), 1).dot(centerWordVec.reshape(1, len(centerWordVec)))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        lossCurrent, gradc, grado = word2vecLossAndGradient(centerWordVectors[word2Ind.get(currentCenterWord)],
                                                            word2Ind[word],
                                                            outsideVectors,
                                                            dataset)
        loss += lossCurrent
